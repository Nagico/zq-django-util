...
class OssStorage(Storage):
    ...
    def __init__(
        ...
        self.access_key_id = (
            access_key_id
            if access_key_id
            else _get_config("ALIYUN_OSS_ACCESS_KEY_ID")
        )
        self.access_key_secret = (
            access_key_secret
            if access_key_secret
            else _get_config("ALIYUN_OSS_ACCESS_KEY_SECRET")
        )
        self.end_point = _normalize_endpoint(
            end_point if end_point else _get_config("ALIYUN_OSS_ENDPOINT")
        )
        self.bucket_name = (
            bucket_name
            if bucket_name
            else _get_config("ALIYUN_OSS_BUCKET_NAME")
        )
        self.expire_time = (
            expire_time
            if expire_time
            else int(
                _get_config("ALIYUN_OSS_EXPIRE_TIME", default=60 * 60 * 24 * 30)
            )
        )
